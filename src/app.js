...
// ✨ FIXED: Create new file - NOW WITH FILE PATH SUPPORT
function createNewFile(name = null, content = '// New file\n', filePath = null) {
    ...
    const newFile = {
        ...
        name: fileName,
        content: content,
        path: filePath, // ✨ ADDED: Store file path for desktop version
...
// ✨ FIXED: Rename current file - CUSTOM DIALOG VERSION (NO PROMPT)
async function renameFile() {
    ...
    // Check if this is a desktop app with file paths
    if (!window.electronAPI || !file.path) {
        // Fallback for web version or files without paths (unsaved files)
        showRenameDialog(file.name, (newName) => {
    ...
    // Desktop version - actually rename the file on disk
    const currentPath = file.path;
    const currentName = file.name;
    
    console.log('🔧 [RENAME] Starting rename for:', currentPath);
    
    // Show custom dialog instead of prompt()
    showRenameDialog(currentName, async (newName) => {
        console.log('🔧 [RENAME] New name entered:', newName);
        ...
        try {
            // Get the directory path and create new full path
            const pathModule = window.require ? window.require('path') : null;
            
            if (!pathModule) {
                showStatus('Path module not available', 'error');
                console.error('🔧 [RENAME] Cannot load path module');
                return;
            }
            
            const dirPath = pathModule.dirname(currentPath);
            const newPath = pathModule.join(dirPath, newName.trim());
            
            console.log('🔧 [RENAME] Old path:', currentPath);
            console.log('🔧 [RENAME] New path:', newPath);
            
            // Use Electron API to rename the file
            const result = await window.electronAPI.renameFile(currentPath, newPath);
            ...
            if (result.success) {
                ...
                showStatus(`✓ File renamed to: ${file.name}`, 'success');
                console.log('🔧 [RENAME] Success! File renamed on disk and in memory');
            } else {
                console.error('🔧 [RENAME] Failed:', result.error);
                showStatus(`Failed to rename: ${result.error}`, 'error');
                showRenameError('Failed to rename file: ' + result.error);
            }
        } catch (error) {
            console.error('🔧 [RENAME] Error during rename:', error);
            showStatus('Error renaming file', 'error');
            showRenameError('Error renaming file: ' + error.message);
        }
    });
}
...
// ✨ ADDED: Handle file opened from Electron (DESKTOP ONLY)
// This ensures file paths are tracked when files are opened
if (window.electronAPI && window.electronAPI.onFileOpened) {
    window.electronAPI.onFileOpened((event, fileData) => {
        console.log('🔧 [FILE-OPEN] Received file from Electron:', fileData.path);
        createNewFile(fileData.name, fileData.content, fileData.path); // Pass the path!
    });
}

// =====================================================
// CUSTOM RENAME DIALOG FUNCTIONS (NO PROMPT)
// These replace prompt() which doesn't work in Electron
// =====================================================

// Show custom rename dialog (replaces prompt())
function showRenameDialog(currentName, callback) {
    ...
    dialog.innerHTML = `
        <div class="modal-overlay" onclick="closeRenameDialog()">
            <div class="modal-content rename-modal" onclick="event.stopPropagation()">
                <div class="modal-header">
                    <h2>Rename File</h2>
                    <button onclick="closeRenameDialog()" class="modal-close">×</button>
                </div>
                <div class="modal-body">
                    ...
                    <input 
                        ...
                        style="width: 100%; padding: 8px 12px; font-family: 'SF Mono', monospace; font-size: 14px; 
                               background: var(--bg-primary); color: var(--text-primary); 
                               border: 1px solid var(--border-color); border-radius: 4px;"
                    >
                </div>
                <div class="modal-footer" style="display: flex; gap: 8px; justify-content: flex-end; margin-top: 16px;">
                    <button onclick="closeRenameDialog()" class="btn-secondary" style="padding: 8px 16px; background: var(--bg-secondary); color: var(--text-primary); border: 1px solid var(--border-color); border-radius: 4px; cursor: pointer; font-size: 14px;">Cancel</button>
                    <button onclick="submitRename()" class="btn-primary" style="padding: 8px 16px; background: #3498db; color: white; border: none; border-radius: 4px; cursor: pointer; font-size: 14px; font-weight: 500;">Rename</button>
                </div>
            </div>
        </div>
    `;
...
// Submit rename (called by button)
function submitRename() {
    const input = document.getElementById('renameInput');
    const newName = input.value.trim();
    
    if (window._renameCallback) {
        window._renameCallback(newName);
    ...
    errorDialog.innerHTML = `
        <div class="modal-overlay" onclick="this.parentElement.remove()">
            <div class="modal-content" onclick="event.stopPropagation()">
                <div class="modal-header">
                    <h2>Rename Failed</h2>
                    <button onclick="this.closest('.modal-overlay').parentElement.remove()" class="modal-close">×</button>
                </div>
                <div class="modal-body">
                ...
                <div class="modal-footer" style="display: flex; justify-content: flex-end; margin-top: 16px;">
                    <button onclick="this.closest('.modal-overlay').parentElement.remove()" class="btn-primary" style="padding: 8px 16px; background: #3498db; color: white; border: none; border-radius: 4px; cursor: pointer; font-size: 14px;">OK</button>
                </div>
            </div>
        </div>
    `;
